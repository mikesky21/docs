...
**Description**: This widget generates a button that, when clicked, opens a payment page for the widget with ID `8`.

**Parameters**:

- `widget_id` (number): The ID of the widget to open the payment page for.
...
```html
<a href="javascript:void(0);" onclick="sendRequestPayment(8, '9695969889')" class="btn btn-primary">Request payment</a>
```

**Description**: This widget generates a button that, when clicked, sends a payment request to the specified phone number for the widget with ID `8`.

**Parameters**:

- `widget_id` (number): The ID of the widget to send the payment request for.
